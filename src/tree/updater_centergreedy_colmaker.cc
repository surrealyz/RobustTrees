 ...
 * \file updater_centergreedy_colmaker.cc
 * \brief use columnwise update to construct a tree
 * \new robust tree building techniques
 * \author Tianqi Chen, Hongge Chen, Huan Zhang, Yizheng Chen
 */
...
DMLC_REGISTRY_FILE_TAG(updater_centergreedy_colmaker);

/*! \brief column-wise update to construct a tree */
class CenterGreedyColMaker: public TreeUpdater {
 public:
 ...
 protected:
  ...
  struct ThreadEntry {
    ...
    GradStats stats_unc;
    GradStats stats_unc_enum_left;
    GradStats stats_unc_enum_right;
    /*! \brief extra statistics of data */
    GradStats stats_extra;
    //std::deque<const Entry*> data_c_left;
    //std::deque<const Entry*> data_c_right;
   ...
   protected:
    ...
    inline void InitData(const std::vector<GradientPair>& gpair,
      ...
      CHECK_EQ(tree.param.num_nodes, tree.param.num_roots)
          << "CenterGreedyColMaker: can only grow new tree";
      ...
      {
        // setup the epsilon range for each feature index
        if (param_.eps_json != "NULL") {
          dmlc::istream fi(
              dmlc::Stream::Create(param_.eps_json.c_str(), "r"));
          fi >> eps_bound_;
          for (auto& element : eps_bound_.items()) {
            EpsilonEntry bound = {element.value()[0], element.value()[1]};
            feature_eps_[stoi(element.key())] = bound;
          }
        } else {
          // every dimension has the same lower and upper
          for (auto& fid: feat_index_) {
            EpsilonEntry bound = {param_.robust_eps, param_.robust_eps};
            feature_eps_[fid] = bound;
          }
        }
      }
    }
    ...
    inline void EnumerateSplitCacheOpt(const Entry *begin,
      ...
      if (param_.robust_training_verbose) {
        ...
        printf ("\n\n\n########################## We are now using the new CENTER greedy xgboost code on feature id %d ##########################\n", int(fid));
        printf("end-begin=%ld\n",end-begin);
        printf("d_step is %d\n",d_step);
      }
      ...
      // get label cnts for everything.
      std::vector<bst_float> all_labels = info.labels_;
      int all_pos_cnt = 0;
      int all_neg_cnt = 0;
      if (param_.robust_training_verbose) {
        printf ("\n feature values:\n");
        ...
        for (const Entry *it = first; counter <= length; it += descent ? -d_step : d_step, counter++) {   //enforce ascent order
          ...
          float cur_y = all_labels[it->index];
          if (cur_y == 1.0) {
            all_pos_cnt++;
          } else {
            all_neg_cnt++;
        ...
        printf("\n");
        printf("\n all_pos_cnt:%u, all_neg_cnt:%u\n",all_pos_cnt,all_neg_cnt);
      }
      ...
      for (auto nid : qexpand) {
        ...
        temp[nid].stats_c_right.Clear();
        temp[nid].c_left_counter = 0;
        temp[nid].stats_unc.Clear();
        temp[nid].stats_unc_enum_left.Clear();
        temp[nid].stats_unc_enum_right.Clear();
        //temp[nid].data_c_left.clear();
        //temp[nid].data_c_right.clear();
      }

      // get lower and upper epsilon bound
      EpsilonEntry fid_bound = feature_eps_[fid];
      float eps_lo = fid_bound.eps_lo;
      float eps_hi = fid_bound.eps_hi;
      if (param_.robust_training_verbose) printf("epsilon is [%4.5f, %4.5f]\n", eps_lo, eps_hi);
      // left statistics
      GradStats c(param_);
      ...
      // prepare data_c_right and stats_c_right
      counter = 1;
      for (const Entry *it = first; counter <= length; it += descent ? -d_step : d_step, counter++) {   //enforce ascent order
        const bst_uint ridx = it->index;
        const int nid = position_[ridx];
        if (nid < 0) continue;
        ThreadEntry &e = temp[nid];
        //e.data_c_right.push_back(it);
        e.stats_c_right.Add(gpair, info, ridx);
      }

      counter = 1;
      const Entry *left_pos = first;
      const Entry *right_pos = first;
      for (const Entry *it = first; counter <= length; it += descent ? -d_step : d_step, counter++) {//enforce ascent order
        ...
        if (it == first) {
          ...
          e.stats.Add(gpair, info, ridx);
          // [right_pos, last_plus)
          for (const Entry *it_right = right_pos; it_right != last_plus; it_right += descent ? -d_step : d_step) {
            const int right_nid = position_[it_right->index];
            if (right_nid < 0) continue;
            ThreadEntry &right_e = temp[right_nid];
            right_pos = it_right;
            if (it_right->fvalue <= eta + eps_lo) {
              /* move from certain right to uncertain */
              //right_e.data_c_right.pop_front();
              right_e.stats_c_right.Subtract(gpair, info, it_right->index);
              //right_e.data_unc_right.push_back(it_right);
              //right_e.stats_unc_right.Add(gpair, info, it_right->index);
              right_e.data_unc.push_back(it_right);
              right_e.stats_unc.Add(gpair, info, it_right->index);
            } else {
              break;
            }
          }
          if (right_pos != last_plus and right_pos->fvalue <= eta + eps_lo) {
            right_pos = last_plus;
          }
          continue;
        } else {
          eta = (fvalue + e.last_fvalue) * 0.5f;
        }

        if (param_.robust_training_verbose) printf("eta %4.5f, uncertainty range: %4.5f ~ %4.5f, gradient:%4.5f, hessian:%4.5f\n", eta, eta-eps_lo, eta+eps_hi, gpair[ridx].GetGrad(), gpair[ridx].GetHess());
        ...
        // [left_pos, it)
        for (const Entry *it_left = left_pos; it_left != it; it_left += descent ? -d_step : d_step) {
          const int left_nid = position_[it_left->index];
          if (left_nid < 0) continue;
          ThreadEntry &left_e = temp[left_nid];
          left_pos = it_left;
          if (it_left->fvalue < eta - eps_hi) {
            /* move from uncertain to certain left */
            left_e.data_unc.pop_front();
            left_e.stats_unc.Subtract(gpair, info, it_left->index);
            //left_e.data_c_left.push_back(it_left);
            left_e.stats_c_left.Add(gpair, info, it_left->index);
            left_e.c_left_counter++;
          } else {
            break;
          }
        }
        if (left_pos->fvalue < eta - eps_hi) {
          left_pos = it;
        }
        ...
        if (fvalue != e.last_fvalue &&
          ...
          if (c.sum_hess >= param_.min_child_weight) {
            ...
            int mixed = 0;
            if (!e.data_unc.empty() && e.data_unc.size()>0){
              if (param_.robust_training_verbose) printf("\n [start minimization]\n");

              bst_float greedy_loss_chg;

              // Stats for the uncertain set
              int pos_cnt = 0;
              int neg_cnt = 0;
              int total_cnt = e.data_unc.size();

              GradStats new_left(param_);
              GradStats new_right(param_);

              if (param_.robust_training_verbose) {
                for (bst_omp_uint i = 0; i < e.data_unc.size(); i++) {
                  const Entry *cur = e.data_unc[i];
                  float cur_y = all_labels[cur->index];
                  if (cur_y == 1.0) {
                    pos_cnt++;
                  } else {
                    neg_cnt++;
                ...
                mixed = total_cnt>pos_cnt && total_cnt>neg_cnt;
                printf("\n mixed=%d, total_cnt:%u, pos_cnt:%u, neg_cnt:%u\n",
                mixed, total_cnt,pos_cnt,neg_cnt);
              }

              /* Greedy minimization
              * iteratively put into the smaller half.
              */
              bst_float one_left_loss_chg;
              bst_float one_right_loss_chg;
              new_left.Clear();
              new_right.Clear();
              new_left.SetCopy(e.stats_c_left);
              //new_right.SetCopy(e.stats_c_right);
              GradStats sanity_right_0;
              GradStats sanity_right;
              sanity_right_0.SetSubstract(snode_[nid].stats, e.stats_c_left);
              sanity_right.SetSubstract(sanity_right_0, e.stats_unc);
              new_right.SetCopy(sanity_right);
              if (param_.robust_training_verbose) {
                printf("e.stats_c_right.sum_grad=%f, e.stats_c_right.sum_hess=%f\n", e.stats_c_right.sum_grad, e.stats_c_right.sum_hess);
                printf("new_right.sum_grad=%f, new_right.sum_hess=%f\n", new_right.sum_grad, new_right.sum_hess);
                printf("sanity_right.sum_grad=%f, sanity_right.sum_hess=%f\n", sanity_right.sum_grad, sanity_right.sum_hess);
                //printf("***e.data_c_left.size=%lu\n", e.data_c_left.size());
                //printf("***e.data_c_right.size=%lu\n", e.data_c_right.size());
              }

              for (bst_omp_uint i = 0; i < e.data_unc.size(); i++) {
                // decide which side to put the current data point.
                // get stats for e.data_unc[i]
                const Entry *cur = e.data_unc[i];

                // left case
                new_left.Add(gpair, info, cur->index);
                if (d_step == -1) {
                  one_left_loss_chg = static_cast<bst_float>(
                      spliteval_->ComputeSplitScore(nid, fid, new_right, new_left) -
                      snode_[nid].root_gain);
                } else {
                  one_left_loss_chg = static_cast<bst_float>(
                      spliteval_->ComputeSplitScore(nid, fid, new_left, new_right) -
                      snode_[nid].root_gain);
                }
                new_left.Subtract(gpair, info, cur->index);

                // right case
                new_right.Add(gpair, info, cur->index);
                if (d_step == -1) {
                  one_right_loss_chg = static_cast<bst_float>(
                      spliteval_->ComputeSplitScore(nid, fid, new_right, new_left) -
                      snode_[nid].root_gain);
                } else {
                  one_right_loss_chg = static_cast<bst_float>(
                ...
                new_right.Subtract(gpair, info, cur->index);

                if (one_left_loss_chg < one_right_loss_chg) {
                  // put left
                  new_left.Add(gpair, info, cur->index);
                  //printf("left ");
                } else {
              ...
              if (d_step == -1) {
                greedy_loss_chg = static_cast<bst_float>(
                    spliteval_->ComputeSplitScore(nid, fid, new_right, new_left) -
                    snode_[nid].root_gain);
              } else {
                greedy_loss_chg = static_cast<bst_float>(
              ...
              if (param_.robust_training_verbose) {
                printf("\n greedy loss change is %4.5f\n", greedy_loss_chg);
              }
              if (greedy_loss_chg < loss_chg) {
                loss_chg = greedy_loss_chg;
        ...
        if (param_.robust_training_verbose) printf("\n [main loop] current best split for nid %d is fid=%d, threshold=%4.5f and with loss change=%4.5f", nid, e.best.SplitIndex(), e.best.split_value, e.best.loss_chg);
      }
      ...
      for (int nid : qexpand) {
        ...
        if (e.stats.sum_hess >= param_.min_child_weight &&
          ...
          const bst_float gap = std::abs(e.last_fvalue) + kRtEps + eps_lo;
          const bst_float delta = d_step == +1 ? gap: -gap;
          e.best.Update(loss_chg, fid, e.last_fvalue + delta, d_step == -1);
        }
        if (param_.robust_training_verbose) {
    ...
    std::unique_ptr<SplitEvaluator> spliteval_;
    json eps_bound_;
    std::map<int, EpsilonEntry> feature_eps_;
  };
};

// distributed column maker
class CenterGreedyDistColMaker : public CenterGreedyColMaker {
 public:
  void Init(const std::vector<std::pair<std::string, std::string> >& args) override {
    param_.InitAllowUnknown(args);
  ...
  void Update(HostDeviceVector<GradientPair> *gpair,
              ...
              const std::vector<RegTree*> &trees) override {
    GradStats::CheckInfo(dmat->Info());
    CHECK_EQ(trees.size(), 1U) << "CenterGreedyDistColMaker: only support one tree at a time";
    Builder builder(
      param_,
      std::unique_ptr<SplitEvaluator>(spliteval_->GetHostClone()));
    // build the tree
 ...
 private:
  class Builder : public CenterGreedyColMaker::Builder {
   public:
    explicit Builder(const TrainParam &param,
                     std::unique_ptr<SplitEvaluator> spliteval)
        : CenterGreedyColMaker::Builder(param, std::move(spliteval)) {}
...
XGBOOST_REGISTER_TREE_UPDATER(CenterGreedyColMaker, "centergreedy_grow_colmaker")
.describe("Grow tree with parallelization over columns.")
.set_body([]() {
    return new CenterGreedyColMaker();
  });

XGBOOST_REGISTER_TREE_UPDATER(CenterGreedyDistColMaker, "centergreedy_distcol")
.describe("Distributed column split version of tree maker.")
.set_body([]() {
    return new CenterGreedyDistColMaker();
  });
}  // namespace tree
}  // namespace xgboost
